...
class ChessGame:
    ...
    def __init__(self, position_str, current_turn, castling_posibilities, en_passant_square):
        self.board = bp.Board()
    ...
    def pawn_of_ep_square(self):
        if self.en_passant_square is None: return None
        file, rank = self.board.file_rank_of_sq(self.en_passant_square)
        forward = None
        if rank==2:
            forward = 1
        if rank==5:
            forward = -1
        pawn_at_sq = self.board.square_of_fr(file, rank+forward)
        return pawn_at_sq if isinstance(pawn_at_sq, bp.Pawn) else None
